[ ![Codeship Status for andywillis/EVL](https://www.codeship.io/projects/fd38b440-0456-0132-4808-06cd9fe8c123/status?branch=master)](https://www.codeship.io/projects/30629)

[![Coverage Status](https://coveralls.io/repos/andywillis/EVL/badge.png?branch=master)](https://coveralls.io/r/andywillis/EVL?branch=master)
[![Dependency Status](https://david-dm.org/andywillis/EVL.png?theme=shields.io)](https://david-dm.org/andywillis/EVL)
[![devDependency Status](https://david-dm.org/andywillis/EVL/dev-status.png?theme=shields.io)](https://david-dm.org/andywillis/EVL#info=devDependencies)
...
- Go into the repo and install all the modules (defined in the `dev-dependancies` section of `package.json`) using the command: `npm install`. A new folder called `node-modules` will appear in the root folder.
- `bower install` will install all the bower components we need to `src/lib/js`.
...
Contains the source code for running the app. It includes an `index.html` file on to which the `connect` process grabs to launch the server. The core JavaScript is stored in `core`, and the additional JavaScript / CSS files are stored in `lib`.
